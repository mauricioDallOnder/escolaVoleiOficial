...
export default function CadastrarAlunoPage() {
  ...
  useEffect(() => {
    async function carregarTurmas() {
      // Aqui você pode filtrar a modalidade "volei" ou outra, conforme sua lógica
      const dados = await fetchModalidades("volei");
      // Esperamos que "dados" seja um array do tipo: [{ nome: "volei", turmas: [...]}]
      const volei = dados.find((mod) => mod.nome === "volei");
      if (volei) {
        setTurmas(volei.turmas); // turmas do volei
      }
    }
    carregarTurmas();
  }, [fetchModalidades]);
    ...
    <Layout>
      <Container>
        <Box sx={BoxStyleCadastro}>
          <Typography sx={TituloDaPagina}>Cadastrar Aluno</Typography>
          ...
          >
            ...
            <Box sx={ListStyle}>
              <Typography sx={TituloSecaoStyle}>
                Seção 8 - Selecione a Turma
              </Typography>
              <FormControl fullWidth error={!!errors.turmaSelecionada}>
